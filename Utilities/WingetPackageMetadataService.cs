...
internal static class WingetPackageMetadataService
{
    ...
    private static async Task<WingetPackageMetadata> LoadAsync(string packageId)
    {
        ...
        try
        {
            ...
            {
                FileName = "winget",
                Arguments = $"show --id \"{EscapeWingetArgument(packageId)}\" --exact --source {(RecommendedAppCatalog.IsMicrosoftStorePackage(packageId) ? "msstore" : "winget")} --accept-source-agreements --disable-interactivity --no-progress",
